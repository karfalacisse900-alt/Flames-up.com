...
fn ffi_or_default<T: Default>(operation: impl FnOnce() -> T) -> T {
...
mod panic_boundary_tests {
    use super::ffi_or_default;

    #[test]
    fn ffi_panics_are_converted_to_default_values() {
        let value: u64 = ffi_or_default(|| panic!("test-only FFI panic"));
        assert_eq!(value, 0);
    }
}
