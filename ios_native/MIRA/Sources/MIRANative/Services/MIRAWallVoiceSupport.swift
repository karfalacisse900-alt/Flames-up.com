...
@MainActor
final class MIRAWallApproximateLocationProvider: NSObject, ObservableObject, CLLocationManagerDelegate {
  ...
  func resolve() {
    ...
    timeoutTask = Task { @MainActor [weak self] in
      ...
      guard !Task.isCancelled, let self, isResolving else { return }
      finishWithError("Location is unavailable right now. You can still post without it.")
    }
    ...
    case .denied, .restricted:
      ...
      finishWithError("Location access is off. You can still post without it.")
    @unknown default:
      finishWithError("Location is unavailable right now. You can still post without it.")
    }
  }
    ...
    Task { @MainActor [weak self] in
      ...
      do {
        ...
        guard let placemark = placemarks.first else {
          finishWithResolvedLocation(city: nil, country: nil)
          return
        }
        let resolvedCity = placemark.locality ?? placemark.subAdministrativeArea ?? placemark.administrativeArea
        finishWithResolvedLocation(city: resolvedCity, country: placemark.country)
      } catch {
        finishWithResolvedLocation(city: nil, country: nil)
      }
    }
  ...
  private func finishWithResolvedLocation(city: String?, country: String?) {
    timeoutTask?.cancel()
    isResolving = false
    self.city = city
    self.country = country
    let publicPlace = city ?? country
    label = publicPlace.map { "Found near \($0)" } ?? "Found nearby"
    errorMessage = nil
  }
