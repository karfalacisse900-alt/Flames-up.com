...
private struct YearbookOpenSpread: View {
  ...
  let myProfile: MIRAYearbookProfile?
    ...
    GeometryReader { proxy in
      ...
      ScrollView(.vertical) {
        VStack(spacing: 13) {
          ...
          YearbookOverviewPanels(profile: myProfile, onEdit: onEdit)
            .padding(.horizontal, max(10, spreadWidth * 0.028))
...
private struct YearbookOverviewPanels: View {
  let profile: MIRAYearbookProfile?
  let onEdit: () -> Void

  var body: some View {
    HStack(alignment: .top, spacing: 10) {
      YearbookLookingForPanel(selectedIntent: profile?.intent)
        .frame(maxWidth: .infinity, maxHeight: .infinity)
      YearbookEditProfilePanel(profile: profile, onEdit: onEdit)
        .frame(maxWidth: .infinity, maxHeight: .infinity)
    }
    .frame(height: 190)
  }
...
private struct YearbookLookingForPanel: View {
  ...
  var body: some View {
    ZStack(alignment: .topLeading) {
      ...
      VStack(alignment: .leading, spacing: 7) {
        HStack(spacing: 5) {
          Text("YOU'RE LOOKING FOR")
            .font(.system(size: 8.2, weight: .bold, design: .serif))
            .tracking(0.25)
            .lineLimit(1)
          ...
          Image(systemName: "star")
            .font(.system(size: 13, weight: .medium))
            .foregroundStyle(Color(red: 0.77, green: 0.34, blue: 0.38))
        }

        ForEach(MIRAYearbookIntent.allCases) { intent in
          HStack(spacing: 7) {
            Image(systemName: icon(for: intent))
              .font(.system(size: 11, weight: .medium))
              .frame(width: 15)

            Text(intent.title)
              .font(.custom("Noteworthy-Bold", size: 10.5, relativeTo: .caption))
              .lineLimit(1)
              .minimumScaleFactor(0.68)
      ...
      YearbookTapeStrip(color: Color(red: 0.76, green: 0.45, blue: 0.50))
        .frame(width: 62, height: 13)
        .offset(x: 51, y: -5)
    }
    .rotationEffect(.degrees(-0.65))
    .accessibilityElement(children: .combine)
    .accessibilityLabel("Looking for \(selectedIntent?.title ?? "not selected")")
  }

  private var overviewPaperStack: some View {
    ZStack {
      YearbookPageTexture(base: Color(red: 0.86, green: 0.82, blue: 0.73), ruled: false)
        .offset(x: -3, y: 5)
      YearbookPageTexture(base: Color(red: 0.92, green: 0.89, blue: 0.82), ruled: false)
        .offset(x: 2, y: 2)
      YearbookPageTexture(base: Color(red: 0.967, green: 0.944, blue: 0.875), ruled: true)
    }
    .clipShape(RoundedRectangle(cornerRadius: 4, style: .continuous))
    .overlay {
  ...
  private func icon(for intent: MIRAYearbookIntent) -> String {
    switch intent {
    case .friends: return "person.2"
    case .dating: return "heart"
    case .friendsAndDating: return "person.2.fill"
    case .creativeNetworking: return "bolt"
    case .justBrowsing: return "binoculars"
    }
  }
}

private struct YearbookEditProfilePanel: View {
  let profile: MIRAYearbookProfile?
  let onEdit: () -> Void
  ...
  var body: some View {
    Button(action: onEdit) {
      VStack(spacing: 0) {
        ...
        ZStack(alignment: .bottomTrailing) {
          YearbookSmallAvatar(url: profile?.profilePhoto, size: 55)
          Image(systemName: "camera")
            ...
            .clipShape(Circle())
            .overlay { Circle().stroke(Color.black.opacity(0.14), lineWidth: 0.7) }
            .offset(x: 5, y: 3)
        }
        .padding(.vertical, 5)

        VStack(spacing: 0) {
          editRow(icon: "pencil", title: "Edit Details")
          editRow(icon: "lock", title: "Privacy Settings")
          editRow(icon: "book.closed", title: "Profile Theme", value: profile?.theme.title ?? "Vintage Paper")
          editRow(icon: "list.clipboard", title: "Yearbook Prompts", value: "\(profile?.prompts?.count ?? 0) added")
        }
      ...
      .frame(maxWidth: .infinity, maxHeight: .infinity)
      .background {
        YearbookPageTexture(base: Color(red: 0.968, green: 0.945, blue: 0.884), ruled: false)
      }
    ...
    .accessibilityLabel("Edit Yearbook profile")
  }

  private func editRow(icon: String, title: String, value: String? = nil) -> some View {
    HStack(spacing: 5) {
      ...
      if let value {
        Text(value)
          .font(.system(size: 7.1, weight: .medium, design: .serif))
          .foregroundStyle(Color.black.opacity(0.52))
          .lineLimit(1)
...
private struct YearbookPageTexture: View {
  ...
  var body: some View {
    ZStack {
      ...
      RadialGradient(
        colors: [Color.clear, Color(red: 0.38, green: 0.24, blue: 0.10).opacity(0.035)],
        center: .center,
        startRadius: 45,
        endRadius: 270
      )
...
private struct YearbookPolaroidPaper: View {
  ...
  var body: some View {
    ZStack {
      ...
      Rectangle()
        .strokeBorder(Color(red: 0.40, green: 0.30, blue: 0.19).opacity(0.16), lineWidth: 0.8)
        .padding(1.5)
    }
  }
}
  ...
  var body: some View {
    Image(systemName: "paperclip")
      ...
      .foregroundStyle(
        LinearGradient(
          colors: [
            Color(red: 0.66, green: 0.64, blue: 0.56),
            Color(red: 0.24, green: 0.23, blue: 0.20),
            Color(red: 0.50, green: 0.48, blue: 0.42),
          ],
          startPoint: .topLeading,
          endPoint: .bottomTrailing
        )
  ...
  var body: some View {
    ZStack(alignment: .bottom) {
      ...
      Capsule()
        ...
        .offset(x: -8)
      Ellipse()
        .fill(Color(red: 0.34, green: 0.45, blue: 0.22).opacity(0.46))
        .frame(width: 9, height: 22)
        .rotationEffect(.degrees(49))
        .offset(x: 3, y: -18)
      Ellipse()
        .fill(Color(red: 0.34, green: 0.45, blue: 0.22).opacity(0.42))
...
private struct YearbookProfileDetailView: View {
    ...
    VStack(alignment: .leading, spacing: 5) {
      HStack(alignment: .firstTextBaseline, spacing: 5) {
        Text(model.profile.name)
          .font(.custom("Noteworthy-Bold", fixedSize: 32))
          .foregroundStyle(Color(red: 0.12, green: 0.095, blue: 0.075))
          .lineLimit(1)
      ...
      Text(model.profile.intent.title)
        ...
        .padding(.horizontal, 9)
        .frame(height: 23)
        .background(Color(red: 0.63, green: 0.73, blue: 0.48).opacity(0.92))
        .clipShape(RoundedRectangle(cornerRadius: 4, style: .continuous))
  ...
  private var scrapbookPromptPanel: some View {
    YearbookPaperScrap(
      ...
      .overlay(alignment: .topTrailing) {
        Circle()
          .fill(Color(red: 0.78, green: 0.13, blue: 0.13))
  ...
  private var aboutMeText: String {
    let value = model.profile.shortBio?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    return value.isEmpty ? "Not shared yet." : value
  }

  private var referenceDetails: [YearbookDetailItem] {
    [
      YearbookDetailItem(label: "Height", value: model.profile.heightCm.map { "\($0) cm" } ?? "Not shared"),
      YearbookDetailItem(label: "Job", value: yearbookValue(model.profile.job)),
      YearbookDetailItem(label: "Languages", value: yearbookValue((model.profile.languages ?? []).joined(separator: " / "))),
      YearbookDetailItem(label: "School", value: yearbookValue(model.profile.school)),
      YearbookDetailItem(label: "From", value: yearbookValue(model.profile.locationLine)),
    ]
  }
  ...
  private var promptAnswer: String {
    yearbookValue(referencePrompt?.answer, fallback: "Not shared yet.")
  }

  private var favoriteSong: String {
    yearbookValue(model.profile.favorites?["song"], fallback: "Not added yet")
  }

  private var favoritePlace: String {
    yearbookValue(model.profile.favorites?["place"], fallback: "Not added yet")
  }

  private func yearbookValue(_ value: String?, fallback: String = "Not shared") -> String {
...
private struct YearbookScrapbookPatinaLayer: View {
  var body: some View {
    ZStack {
      LinearGradient(
        colors: [
      ...
      RadialGradient(
        colors: [Color.clear, Color.clear, Color(red: 0.33, green: 0.22, blue: 0.11).opacity(0.08)],
        center: .center,
        startRadius: 110,
        endRadius: 330
      )
      ...
      Canvas { context, size in
        for index in 0..<26 {
          let x = CGFloat((index * 83 + 29) % 389) / 389 * size.width
          let y = CGFloat((index * 59 + 41) % 601) / 601 * size.height
          let width = CGFloat(8 + index % 18)
          var scuff = Path()
          scuff.move(to: CGPoint(x: x, y: y))
          scuff.addCurve(
            to: CGPoint(x: min(size.width, x + width), y: y + CGFloat(index % 3) - 1),
            control1: CGPoint(x: x + width * 0.35, y: y - 1),
            control2: CGPoint(x: x + width * 0.68, y: y + 1)
          )
          context.stroke(scuff, with: .color(Color.black.opacity(0.018)), lineWidth: 0.7)
        }
      }
    }
    .allowsHitTesting(false)
...
private struct YearbookScrapbookDepthLayer: View {
  var body: some View {
    ZStack {
      LinearGradient(
        colors: [
          ...
          Color.clear,
          Color(red: 0.32, green: 0.20, blue: 0.09).opacity(0.055),
        ],
        startPoint: .topLeading,
        endPoint: .bottomTrailing
      )
      ...
      Canvas { context, size in
        let edgeColor = Color(red: 0.35, green: 0.23, blue: 0.11).opacity(0.085)
        let cornerSize = min(size.width, size.height) * 0.30

        context.fill(
          Path(ellipseIn: CGRect(x: -cornerSize * 0.55, y: -cornerSize * 0.52, width: cornerSize, height: cornerSize)),
          with: .radialGradient(
            Gradient(colors: [edgeColor, Color.clear]),
            center: CGPoint(x: 0, y: 0),
            startRadius: 1,
            endRadius: cornerSize * 0.55
          )
        )
        context.fill(
          Path(ellipseIn: CGRect(x: size.width - cornerSize * 0.45, y: size.height - cornerSize * 0.48, width: cornerSize, height: cornerSize)),
          with: .radialGradient(
            Gradient(colors: [edgeColor, Color.clear]),
        ...
        for index in 0..<34 {
          let x = CGFloat((index * 113 + 23) % 397) / 397 * size.width
          let y = CGFloat((index * 71 + 17) % 613) / 613 * size.height
          let diameter = CGFloat(0.7 + Double(index % 3) * 0.45)
          context.fill(
            Path(ellipseIn: CGRect(x: x, y: y, width: diameter, height: diameter)),
            with: .color(Color(red: 0.30, green: 0.20, blue: 0.11).opacity(0.065))
          )
        }
      }
      ...
      RoundedRectangle(cornerRadius: 8, style: .continuous)
        ...
        .padding(2)
      RoundedRectangle(cornerRadius: 8, style: .continuous)
        .strokeBorder(Color(red: 0.33, green: 0.22, blue: 0.12).opacity(0.20), lineWidth: 1)
        .padding(5)
    }
    .allowsHitTesting(false)
  }
  ...
  var body: some View {
    Canvas { context, size in
      for x in stride(from: 0.0, through: size.width, by: 12.0) {
        ...
        context.stroke(line, with: .color(Color.blue.opacity(0.105)), lineWidth: 0.6)
      }
  ...
  var body: some View {
    Button(action: action) {
      ...
      .background {
        if filled {
          LinearGradient(
            colors: [Color(red: 0.16, green: 0.15, blue: 0.13), Color(red: 0.10, green: 0.095, blue: 0.08)],
            startPoint: .top,
            endPoint: .bottom
          )
        } else {
          YearbookPageTexture(base: Color(red: 0.977, green: 0.947, blue: 0.875), ruled: false)
        }
      }
      .clipShape(RoundedRectangle(cornerRadius: 3))
      .overlay {
        RoundedRectangle(cornerRadius: 3).stroke(filled ? Color.white.opacity(0.05) : Color.black.opacity(0.18), lineWidth: 0.9)
      }
