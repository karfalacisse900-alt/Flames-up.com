...
const migration = await readFile(new URL('../../supabase/migrations/20260904231905_stripe_native_payments.sql', import.meta.url), 'utf8');
// Historical migrations omit the original baseline. Use the actual deployed DDL,
// without rows, and apply the pending migration to that empty isolated database.
await writeFile(join(directory, files[0]), `${schema}\n${migration}`);
console.log('Prepared schema-only snapshot plus pending native-payment migration. No production rows copied.');
