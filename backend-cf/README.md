...
3. Configure vars:
   ...
   - `SUPABASE_SERVICE_ROLE_KEY` as a Worker secret only; never put this in Expo/frontend env
...
The frontend should only use public Supabase values:
...
- Supabase Auth redirect URLs should include both `https://flames-up.com/auth/callback` and `frontend://auth/callback`
