...
Existing `users.is_admin = 1` accounts are treated as `admin`. Usernames listed in `OWNER_USERNAMES` are treated as `owner`. Use the `admin_roles` table for explicit lower-privilege roles.
