-- Harden like uniqueness and repair counters.
-- This keeps one like row per user/object even on older production databases.

DELETE FROM likes
WHERE id IN (
  SELECT id FROM (
    SELECT id,
           ROW_NUMBER() OVER (PARTITION BY user_id, post_id ORDER BY created_at ASC, id ASC) AS rn
    FROM likes
  )
  WHERE rn > 1
);
...
WHERE id IN (
  SELECT id FROM (
    SELECT id,
           ROW_NUMBER() OVER (PARTITION BY user_id, comment_id ORDER BY created_at ASC, id ASC) AS rn
    FROM comment_likes
  )
  WHERE rn > 1
);
...
WHERE id IN (
  SELECT id FROM (
    SELECT id,
           ROW_NUMBER() OVER (PARTITION BY status_id, user_id ORDER BY created_at ASC, id ASC) AS rn
    FROM status_likes
  )
  WHERE rn > 1
);
