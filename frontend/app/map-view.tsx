...
import { clusterPlaces, rankPlacesByQuery } from '../src/utils/geoSpatial';
...
export default function MapViewScreen() {
  ...
  const filteredPlaces = useMemo(() => {
    ...
    return rankPlacesByQuery(places, query, { lat, lng }, 200);
  }, [places, searchQuery, lat, lng]);

  const clusteredNodes = useMemo(() => {
    const nodes = clusterPlaces(filteredPlaces, 14);
    return nodes.map((node, index) => {
      if (node.kind === 'place') {
        const place = node.place;
        const idx = filteredPlaces.findIndex((p) => {
          const left = String(p.place_id || p.id || '');
          const right = String(place.place_id || place.id || '');
          return left && right ? left === right : p === place;
        });
        return {
          kind: 'place',
          id: node.id,
  ...
  useEffect(() => {
    ...
    const stillVisible = filteredPlaces.some(
      (p) => String(p.place_id || p.id) === String(selected.place_id || selected.id)
    );
  ...
  const handleMsg = (e: any) => {
    try {
      ...
      if (d.type === 'select' && d.idx !== undefined && d.idx >= 0) {
        setSelected(filteredPlaces[d.idx]);
      }
  ...
  const mapHTML = useCallback(() => {
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .cluster-pin{min-width:42px;height:42px;border-radius:21px;padding:0 10px;display:flex;align-items:center;justify-content:center;background:#1A1A1A;color:#fff;border:2px solid #fff;font-family:-apple-system,sans-serif;font-size:13px;font-weight:800;box-shadow:0 4px 14px rgba(0,0,0,.3)}
    .pin-badge{position:absolute;bottom:-2px;right:-2px;background:#1A1A1A;color:#fff;font-size:9px;font-weight:800;padding:2px 5px;border-radius:8px;font-family:-apple-system,sans-serif;border:1.5px solid #fff}
    ...
    <SafeAreaView style={s.container} edges={['top']}>
      <View style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.subtitle}>{filteredPlaces.length} places nearby</Text>
        </View>
      ...
      <View style={s.searchWrap}>
        ...
        <TextInput
          ...
          placeholder="Search places fast..."
      ...
      <View style={s.mapWrap}>
        {loading && places.length === 0 ? (
          ...
          <iframe srcDoc={mapHTML()} style={{ width: '100%', height: '100%', border: 'none' } as any} title="Map" />
        ) : WebView ? (
          <WebView
            ref={wvRef}
            source={{ html: mapHTML() }}
        ...
        <View style={s.zoomBtns}>
          <TouchableOpacity
            style={s.zoomBtn}
            onPress={() => wvRef.current?.injectJavaScript('window.__mapRef&&window.__mapRef.setZoom(window.__mapRef.getZoom()+1);true;')}
          >
              ...
              <View style={s.cardInfo}>
                ...
                <View style={s.cardMeta}>
                  ...
                  <View
                    style={[
                      s.statusPill,
                      { backgroundColor: selected.open_now !== false ? '#DCFCE7' : '#FEE2E2' },
                    ]}
                  >
                    <View
                      style={[
                        s.statusDot,
                        { backgroundColor: selected.open_now !== false ? '#16A34A' : '#DC2626' },
                      ]}
                    />
                    <Text
                      style={[
                        s.statusLabel,
                        { color: selected.open_now !== false ? '#16A34A' : '#DC2626' },
                      ]}
                    >
