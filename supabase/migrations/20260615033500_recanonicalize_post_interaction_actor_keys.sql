-- Re-canonicalize post interaction identity after moving Captro to Supabase
-- primary data. Older Worker builds could write UUID-shaped app user ids as
-- auth actor keys, which made one human look like multiple likers after cache
-- refreshes. Prefer the linked Supabase auth user, then provider identity, then
-- app user id, and rebuild counters from that single actor key.

alter table public.app_post_interactions
  add column if not exists actor_key text;

drop index if exists public.app_post_interactions_legacy_actor_unique;
drop index if exists public.app_post_interactions_native_actor_unique;
...
set
  ...
  user_id = coalesce(c.canonical_auth_user_id, i.user_id),
  updated_at = now()
from canonical c
where i.ctid = c.ctid
  ...
  and (
    ...
    or (c.canonical_auth_user_id is not null and i.user_id is distinct from c.canonical_auth_user_id)
  );
...
with actor_counts as (
  ...
  left join public.app_post_interactions i
    ...
    or i.post_id = p.id
  group by p.id, p.legacy_post_id
)
