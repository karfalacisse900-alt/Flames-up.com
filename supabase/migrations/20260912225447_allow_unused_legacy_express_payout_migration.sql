-- A legacy Express account that has no money, payout history, or external
-- payout destination is safe to retire even if the seller began its hosted
-- onboarding. Leaving those partial profiles blocked would route people back
-- to the Express sign-in screen that Captro no longer uses.
...
begin
  ...
  -- KYC/profile fields alone are not a payout. An external destination is:
  -- never switch its mapping without a manual financial review.
  if v_current.eligible_debit_card_exists is distinct from false
  ...
  -- Refuse to remap anything that has an application financial record. The
  -- worker independently rechecks Stripe balances, transfers, payouts, and
  -- external accounts immediately before this guarded transaction.
  if exists (
