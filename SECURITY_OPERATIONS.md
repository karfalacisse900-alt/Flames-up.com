...
- Supabase: authentication bridge and Postgres sync. Frontend may use publishable/anon keys only; service-role keys are backend-only.
...
- Mapbox: place search/map display. The frontend token is public-scoped; backend proxy should be preferred for higher-risk/location-heavy flows.
...
- Agora: calls. App certificate is backend-only; frontend receives only short-lived generated call tokens.

No analytics/tracker SDK such as Sentry, Segment, Amplitude, Mixpanel, Firebase Analytics, or Meta Pixel was found in the active frontend/backend source during this pass.
...
- Keep `.env` files out of git; frontend may only use `EXPO_PUBLIC_*` values that are safe to ship.
